...
data_folder_output = Path("output/concentration_convertion/S1/PA_2/")
...
outputName = "_S1_PA_1e3_U1_concentration_convertion"
outputName_plots = "\S1_PA_1e3_U1_concentration_convertion"
lakeApendice = "_LakeS1_ml.txt"

### set scenario
scenario = 1## set scenario  to lake 1 or 2
...
daysSimulation = 30#number of days to simulate
stepsize = 60*60#define size of the calculation time step, in seconds. !! stepsize needs to be smaller than sec_day !! otherwise, code will not run and error message will be printed
##############
...
compartments = ["surface", "epilimnion", "mixed_layer","hypolimnion", "sediment"] #define environmental compartments


compartments_labels = ["1", "2", "3", "4", "5"] #define abbreviations of compartments (used later for indexing/assigning rate constants)
...
compartments_prop=generateFinalDataFrame(DayStart, LastDay, INPUT, data_folder,lakeApendice )
...
processinfoFile = data_folder / "process_info_mixed_layer_as_epi_S1_agg_and_frag_adjusted.txt"
process_prop = processData(processinfoFile)
dailyInputData = data_folder / "Input_day_emissions_1e3.csv"
...
[RateConstants_rowNames, Concentrations_rowNames, Basic_Names, Inflow_rowNames] = preprocessTags(compartments_labels,MPforms_labels)
...
for daySec in range(len(t_days)-1):
    ...
    [Surface, Epilimnion, Mixed_layer, Hypolimnion, Sediment] = preProcessLayers(INPUT,compartments_prop,compartments, dayComputation)
   ...
   # Select column contents by column name using [] operator
    ...
    RateConstants = fillRCmatrix.fillRCmatrix(RateConstants, MP1, SPM1, MP1_BF, 
                                              ...
                                              Epilimnion, Mixed_layer, Hypolimnion, Sediment, 
                                              process_prop)
    ...
    #solve model dynamically
        ...
        #inflow_vector= inflow_vector/timesteps#divide daily inputs over timesteps
        print(inflow_vector)
        wat = 0
        DynamicResults = diffEq(inflow_vector, Concentrations_t0,
                                ...
                                timesteps,dayComputation, scenario, outputName, data_folder_output, Epilimnion, Mixed_layer, Hypolimnion)
...
finalPlotting(stepsize, outputName, data_folder_output)
finalPlottingLOG(stepsize, outputName, data_folder_output) #plot results on log scale
